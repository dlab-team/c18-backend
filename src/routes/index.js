...
export default (app) => {
  ...
  app.use('/empleos', requiereAuth, rolAccess({ roles: ["user", "admin"] }), empleosRoutes);
  app.use('/metas', requiereAuth, rolAccess({ roles: ["user", "admin"] }), metas);
  app.use('/notas', requiereAuth, rolAccess({ roles: ["user", "admin"] }), notasRoutes);
  app.use(
    '/entrevistas',
    requiereAuth,
